...
# Menghitung zona UTM
zone = int((np.degrees(lambda_) + 180) // 6) + 1

# Menentukan belahan bumi (N atau S)
hemisphere = 'N' if phi >= 0 else 'S'
...
E = E0 + k0 * a * (eta_prime + sum(alpha[j-1] * np.cos(2 * j * xi_prime) * np.sinh(2 * j * eta_prime) for j in range(1, 5)))

# Menghitung N (Northing)
N = N0 + k0 * a * (xi_prime + sum(alpha[j-1] * np.sin(2 * j * xi_prime) * np.cosh(2 * j * eta_prime) for j in range(1, 5)))
